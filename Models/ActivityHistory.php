...
class ActivityHistory extends BaseModel
{
    ...
    public static function getMapTitleActivities(): array
    {
        return [
            'voice_active' => [
                ...
                'Собери в голосовом канале отряд интелектуалов',
                'Раскажи шутку ребятам в голосовом канале'
            ],
            'message_active' => [
                'Задай глупый вопрос в чате',
                'Напиши хорошие слова об админе',
                'Напиши интересный факт в чате',
                'Подними философскую тему в чате',
            ...
            'reaction_active' => [
                ...
                'Поставь реакцию с котиком',
                'Зацени рандомный пост',
            ...
            'music_active' => [
                ...
                'Включи мызыку 80-x',
                'Включи музыку дальнобойщиков',
                'Включи Дору',
                'Включи СлавуМерлоу',
                'Включи Шипа',
                'Включи песни МейБиБейби 🎼'
            ],
            'always_active' => [
                'Будь с нами!) (уже выполнено)',
                'ХАЛЯВА 😃 (уже выполнено)',
                'Админ сегодня щедрый (уже выполнено)',
                'Праздник к нам приходит (уже выполнено)',
            ],
        ];
    }
    ...
    /**
     * Возвращает активности на последнюю дату
     *
     * @param integer $discordId
     * @return ActivityHistory|null
     */
    public static function getActivitiesByUser(int $discordId): ?ActivityHistory
    {
        $pdo = self::getPDO();
        if($pdo === null){
            LogService::setLog('Ошибка подключения к базе данных');
        ...
        $stmt = $pdo->prepare("SELECT * FROM activity_history WHERE `discord_id`=:discord_id ORDER BY date DESC");
        ...
        $active = new Self();
        $active->id = $result['id'];
        $active->discord_id = $result['discord_id'];
        $active->date = $result['date'];
        $active->voice_active = $result['voice_active'];
        $active->message_active = $result['message_active'];
        $active->like_active = $result['like_active'];
        $active->mem_active = $result['mem_active'];
        $active->reaction_active = $result['reaction_active'];
        $active->music_active = $result['music_active'];
        $active->always_active = $result['always_active'];
